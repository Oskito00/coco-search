...
Revision ID: 5155fcc1b574
Revises: 
Create Date: 2025-03-26 09:06:21.964835
...
def upgrade():
    ...
    op.execute("""
        ...
        START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1;
    """)
    op.execute("""
        CREATE SEQUENCE item_relevance_feedback_id_seq
        START 1 INCREMENT 1 NO MINVALUE NO MAXVALUE CACHE 1;
    """)
    ...
    sa.Column('keyword_id', sa.BigInteger(), server_default=sa.text("nextval('keywords_keyword_id_seq'::regclass)"), nullable=False),
    ...
    sa.Column('telegram_chat_ids', postgresql.JSONB(astext_type=sa.Text()).with_variant(sa.JSON(), 'sqlite'), nullable=True),
    sa.Column('telegram_connected', sa.Boolean(), nullable=True),
    sa.Column('telegram_notifications_enabled', sa.Boolean(), nullable=True),
    sa.Column('notification_preferences', postgresql.JSONB(astext_type=sa.Text()).with_variant(sa.JSON(), 'sqlite'), nullable=True),
    ...
    sa.Column('tier', postgresql.JSONB(astext_type=sa.Text()).with_variant(sa.JSON(), 'sqlite'), nullable=True),
    sa.Column('subscription_status', sa.String(length=20), nullable=True),
    sa.Column('current_period_end', sa.DateTime(), nullable=True),
    sa.Column('requested_change', postgresql.JSONB(astext_type=sa.Text()).with_variant(sa.JSON(), 'sqlite'), nullable=True),
    sa.Column('pending_tier', postgresql.JSONB(astext_type=sa.Text()).with_variant(sa.JSON(), 'sqlite'), nullable=True),
    ...
    sa.Column('id', sa.BigInteger(), server_default=sa.text("nextval('item_relevance_feedback_id_seq'::regclass)"), autoincrement=True, nullable=False),
...
def downgrade():
    ...
    op.execute("DROP SEQUENCE IF EXISTS keywords_keyword_id_seq;")
    op.execute("DROP SEQUENCE IF EXISTS item_relevance_feedback_id_seq;")
